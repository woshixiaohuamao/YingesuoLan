...
def save_results(data, full_pred, metrics_df, output_dir):
    ...
    results_df = pd.DataFrame({
        'timestamp': full_pred.index,  # 使用预测值的时间索引
        'true_value': data.reindex(full_pred.index).values,  # 重新索引对齐
...
def main():
    # 配置参数
    DATA_PATH = r'D:\Aa中工互联\工作安排\英格索兰\code\预测\lstm\data\data_0607.xlsx'
    MODEL_PATH = r'D:\Aa中工互联\工作安排\英格索兰\code\预测\lstm\sarima_results\最优超参数结果\sarima_model_mse.pkl'
