...
file_name = r'D:\Aa中工互联\工作安排\英格索兰\code\预测\lstm\data\残差修正数据集.xlsx'
...
def evaluate_model(model, X, y_true, sarima_pred, name):
    ...
    mse = mean_squared_error(y_true, y_final_pred)  # 新增MSE指标
    
    # 原始SARIMA的误差
    sarima_rmse = np.sqrt(mean_squared_error(y_true, sarima_pred))
    sarima_mse = mean_squared_error(y_true, sarima_pred)  # 新增原始SARIMA的MSE
    ...
    print(f"相比原始SARIMA提升: "
          ...
          f"MSE提升: {(sarima_mse - mse)/sarima_mse*100:.2f}%")  # 同时显示MSE提升
    
    return y_final_pred, rmse, mae, mape, mse  # 返回新增的MSE
...
print("测试集预测对比图已保存为: test_comparison.png")
...
print("残差分布图已保存为: residual_distribution.png")
