...
'''def interpolate_to_minute(preds, freq='1T'):
    preds = np.array(preds)
    timestamps = pd.date_range(start=0, periods=len(preds), freq='1H')
    df = pd.DataFrame({'timestamp': timestamps, 'value': preds})
...
def convert_to_json_format(df, lower_bounds, upper_bounds, input_df, metric="瞬时流量"):
    ...
    # 生成完整预测时间序列（和df行数对应）
    freq = pd.infer_freq(df['timestamp'])  # 如果df带timestamp，可以推断
    if freq is None:  
        freq = '1T'
