...
df_clean[features] = df_clean[features].replace([np.inf, -np.inf], np.nan).fillna(method='ffill')
...
# 创建未来数据框（包含验证集和测试集）
future = pd.concat([val_df, test_df], axis=0)[['ds'] + features].copy()
future = future.dropna()  # 删除缺失值
...
# 3. 测试集预测对比（新增 SARIMIA 曲线）
plt.figure(figsize=(15, 6))
test_sample = test_results.iloc[:100]  
...
# 图表设置
plt.legend()
plt.title('测试集预测效果对比（Prophet vs SARIMIA）')
plt.xlabel('时间')
plt.ylabel('值')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('test_comparison_sarimia.png', dpi=300)
print("测试集预测对比图（含 SARIMIA）已保存为: test_comparison_sarimia.png")
...
print("模型已保存为: prophet_model.json")