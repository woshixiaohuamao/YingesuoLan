...
# 数据准备 --------------------------------------------------------------
file_path = r'D:\Aa中工互联\工作安排\英格索兰\data\特征工程\滞后特征AVS.xlsx'
df = pd.read_excel(file_path)

# 移除非特征列
features = df.drop(columns=['AVS总流量'])
target = df['AVS总流量']
...
# 构造模型流水线 -------------------------------------------------------
pipeline = Pipeline([
    ('scaler', StandardScaler()),  # 再次标准化（为交叉验证独立处理）
...
joblib_file = "xgb_avs_LAG.pkl" #由于采用了pipline，所以可以实现自动标准化
