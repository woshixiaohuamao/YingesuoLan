...
features = df.drop(columns=['AVS总流量','时间'])  # 移除目标变量和时间列
target = df['AVS总流量']

# 数据集划分（保持时间顺序，适合时间序列数据）
...
X_test_scaled = scaler.transform(X_test)        # 测试集应用相同变换

# 转换回DataFrame（可选，仅用于查看）
...
xgb_reg = xgb.XGBRegressor(
    ...
    n_estimators=278,        # 增加树的数量提升表现
    max_depth=8,             # 适当增加深度
    learning_rate=0.017589157624410087,      # 调低学习率
    subsample=0.6,
